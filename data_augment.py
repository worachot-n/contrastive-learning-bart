...
def synonym_replacement(words, sentence_words, n):
    new_words = words.copy()
    new_sent_words = sentence_words.copy()
    ...
    for random_word in random_word_list:
        ...
        if len(synonyms) >= 1:
            ...
            new_sent_words = [synonym if word == random_word else word for word in new_sent_words]
    ...
    topic = ' '.join(new_words)
    sentence = ' '.join(new_sent_words)
    new_words = topic.split(' ')
    new_sent_words = sentence.split(' ')

    return new_words, new_sent_words
...
def data_augment(topic, sentence, alpha_sr=0.3, alpha_ri=0.2, alpha_rs=0.1, p_rd=0.15, num_aug=9):

    sentence = get_only_chars(sentence)
    topic = get_only_chars(topic)
    ...
    n_sr = max(1, int(alpha_sr*num_keywords))
    n_ri = max(1, int(alpha_ri*num_words))
    n_rs = max(1, int(alpha_rs*num_words))

    #sr
    for _ in range(num_new_per_technique):
        a_words, sent_words = synonym_replacement(keywords, words, n_sr)
        augmented_sentences.append(' '.join(sent_words))
        augmented_topic.append(' '.join(a_words))
    ...
    augmented_topic.append(topic)
    augmented_sentences.append(sentence)

    return augmented_topic, augmented_sentences
